...
\def\exp#1{e^{#1}} %指数関数
\def\prob#1{\operatorname{Pr} \left( #1 \right)} %確率
\def\cprob#1#2{\operatorname{\Pr} \left( #1 \mid #2 \right)} %条件付確率
...
\section{$Transient\quad Probability$}
	前章では架空人物を入れたシステムにおける系内客数の変化を見た．
	この章では時間$(0,t]$において実際の系内客数の状態推移の確率を計算する．
    $Q(0)=i$とせよ．$t$時間後に系内客数が$Q(t)=j$となっている確率は，$Chapman-Kolmogorov$方程式の章での表記に倣って
    $P_{ij}(t) = \cprob{Q(t)=j}{Q(0)=i}$と表される．状態$i$から$j$までに$Q(t)$が上昇する回数と下降する回数をそれぞれ
    ...
    $Q(t)$の変化時間間隔を無視してステップ数だけに注目すると，$u$回の上昇と$d$回の下降を経て状態$j$に行き着くまでの経路の数は
    \begin{eqnarray}
    	\frac{(u+d)!}{u!d!}
    \end{eqnarray}
    通りある．$$
