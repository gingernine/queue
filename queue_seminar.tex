...
\section{$References$}
    ...
    \begin{enumerate}
        ...
        \item Li, Hui, Endo, Kishimoto, A Quantitative Model for Intraday Stock Price
         ...
         \item Ito, Introduction To Lebesgue Integration, Shokabo, 2013, pp. 64-65.
    \end{enumerate}
...
\subsection{$Random\quad Arrival$}
    \begin{description}
    	\item[ランダムな到着]\mbox{}\\
        	...
        	つまり十分大きな時間経過を考えて，確率が1に近いところで$A_{(0, T]} = \lambda T + o(T)$も成立する．精しくは$Egorov$の定理((\ref{sec:appendix_egorov})参照)による．
            到着数の分布は次のように表される．\\
            $A_{(0, T]}$についての仮定と$Egorov$の定理により，任意の$\epsilon > 0,\ \eta > 0$と十分大きな$T$に対して
...
\label{sec:appendix_egorov}
    ...
    \end{screen}
    \begin{Proof}
    	先の定理では一様収束についての言及が無いことに注意するべきである．\\
        ...
        \end{align}
        が成り立ち，かつ$H$の上では任意の$\epsilon > 0$に対して \frac{1}{2^m}
        \begin{align}
        	|f_n(x) - f(x)| < \epsilon
        \end{align}
