...
\subsection{指値/成行注文の時間間隔}
    ...
    ひとまずは簡単のために，一回の指値/成行の注文数は平均株数を一単位と考える(\cite{endo_zuo_kishimoto}を参照)．理論部分では系内客数が$0$になって以降も
    架空人物のサービス時間を考慮する操作が入ったけれども，板の解析は初期値(上下最良気配板の移動直後の板の厚み)から上下どちらかの板の厚みが$0$になるまでのみを
    考え，$0$になって以降は板の移動が起きるので，待ち行列としても新規のシステムを考えることになる．
