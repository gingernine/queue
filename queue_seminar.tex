...
\underline{\large 上下板の消滅時間の分布}\\
    ...
    \begin{align}
    	\Exp{T_A} &= \begin{cases}
        	...
        	\infty. & \rho_A \geq 1
        \end{cases} \label{eq:mean_adminishingtime} \\
        \Exp{T_B} &= \begin{cases}
...
\underline{\large 板が移動する時間間隔の分布}\\
    ...
    \begin{align}
        ...
        &= \int_{0}^{\infty} t f_U(t) dt + \int_{0}^{\infty} t f_D(t) dt. \label{eq:fluctime_TM}
    \end{align}
...
\underline{\large 転移行列}\\
    ...
    \begin{align}
        ...
        p_{DD} &= \int_{0}^{\infty} f_D(t \mid r_A^D, r_B^D) dt = \int_{0}^{\infty} f_B(t \mid r_B^D)dt - \int_{0}^{\infty} f_B(t \mid r_B^D) \left( \int_{0}^{t} f_A(\tau \mid r_A^D)d\tau \right) dt. \label{eq:transientprobs}
    \end{align}
...
\underline{\large 初期デプス}\\
    ...
    板の移動直後の厚みの頻度図を見ると，これは或る確率分布に従っていると推測される．今回は{\rm Poisson}分布，負の二項分布，幾何分布の
    三つの分布に基づく最尤推定を行う．また最尤法により推定したパラメータで計算される確率関数と実際の観測された分布を下図に示す．
    \begin{description}%確率関数
    	\item[{\rm Poisson}分布の確率関数]
    		\begin{align}
    			f(x \mid \lambda) = \frac{\lambda^x}{x!} \exp{-\lambda}, \quad(\lambda > 0,\ x=0,1,2,3,\cdots).
    		\end{align}
        \item[負の二項分布の確率関数]
    		\begin{align}
    			f(x \mid n, p) = \binom{n+x-1}{x} p^n (1-p)^x, \quad(n \in \mathbb{N},\ p \in [0, 1],\ x=0,1,2,3,\cdots).
    		\end{align}
        \item[幾何分布の確率関数]
    		\begin{align}
    			f(x \mid p) = f(x \mid n=1, p) = p (1-p)^x, \quad(p \in [0, 1],\ x=0,1,2,3,\cdots).
    		\end{align}
    \end{description}
    
    これらの分布は$0$から始まる非負整数集合上で定義される特異分布である．板の分析で考える初期デプスは自然数しか取らない確率変数と見做すので
    \begin{align}
    	\prob{R-1=x} = f(x), \quad(\mbox{$R$は初期デプスを表す確率変数，$f$は上記の確率関数．})
    \end{align}
    の関係を満たすように最尤推定法を適用する．
    \begin{table}[H]
    	\centering
        \caption{{\rm Poisson}分布推定パラメータ}
        \begin{tabular}{l||l|l} \bhline{1.5pt}
        	{\rm } & 板上昇直後 & 板下降直後 \\ \hline
        	{\rm } & $\lambda$ & $\lambda$ \\ \hline \hline
			{\rm Best bid} & $39.29340868$ & $420.7710303$ \\ \hline
			{\rm Best ask} & $419.484997$ & $39.31588898$ \\ \bhline{1.5pt}
        \end{tabular}
        \label{tab:initial_depth_nbinom_parameters}
    \end{table}
    
    \begin{table}[H]
    	\centering
        \caption{負の二項分布推定パラメータ}
        \begin{tabular}{l||ll|ll} \bhline{1.5pt}
        	{\rm } & 板上昇直後 & & 板下降直後 & \\ \hline
        	{\rm } & $n$ & $p$ & $n$ & $p$ \\ \hline \hline
			{\rm Best bid} & $0.5555134$ & $0.013940488$ & $3.893347334$ & $0.009168057$ \\ \hline
			{\rm Best ask} & $3.843306821$ & $0.009078785$ & $0.55026574$ & $0.01380283$ \\ \bhline{1.5pt}
        \end{tabular}
        \label{tab:initial_depth_poisson_parameters}
    \end{table}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{l||l|l} \bhline{1.5pt}
        	{\rm } & 板上昇直後 & 板下降直後 \\ \hline
        	{\rm } & $p$ & $p$ \\ \hline \hline
			{\rm Best bid} & $0.024817955$ & $0.002370955$ \\ \hline
			{\rm Best ask} & $0.002378206$ & $0.024804116$ \\ \bhline{1.5pt}
        \end{tabular}
        \label{tab:initial_depth_geom_parameters}
    \end{table}
    
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_up_poiss_bid.depth.eps}
        \end{center}
        \caption{板上昇直後初期デプス({\rm Poisson}分布, {\rm Best bid})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_up_poiss_ask.depth.eps}
        \end{center}
        \caption{板上昇直後初期デプス({\rm Poisson}分布, {\rm Best ask})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_down_poiss_bid.depth.eps}
        \end{center}
        \caption{板下降直後初期デプス({\rm Poisson}分布, {\rm Best bid})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_down_poiss_ask.depth.eps}
        \end{center}
        \caption{板下降直後初期デプス({\rm Poisson}分布, {\rm Best ask})}
    \end{figure}
    
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_up_nbinm_bid.depth.eps}
        \end{center}
        \caption{板上昇直後初期デプス(負の二項分布, {\rm Best bid})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_up_nbinm_ask.depth.eps}
        \end{center}
        \caption{板上昇直後初期デプス(負の二項分布, {\rm Best ask})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_down_nbinm_bid.depth.eps}
        \end{center}
        \caption{板下降直後初期デプス(負の二項分布, {\rm Best bid})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_down_nbinm_ask.depth.eps}
        \end{center}
        \caption{板下降直後初期デプス(負の二項分布, {\rm Best ask})}
    \end{figure}
    
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_up_geome_bid.depth.eps}
        \end{center}
        \caption{板上昇直後初期デプス(幾何分布, {\rm Best bid})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_up_geome_ask.depth.eps}
        \end{center}
        \caption{板上昇直後初期デプス(幾何分布, {\rm Best ask})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_down_geome_bid.depth.eps}
        \end{center}
        \caption{板下降直後初期デプス(幾何分布, {\rm Best bid})}
    \end{figure}
    \begin{figure}[H]
        \begin{center}
    	\includegraphics[clip,width = 10.0cm]{graphics/after_down_geome_ask.depth.eps}
        \end{center}
        \caption{板下降直後初期デプス(幾何分布, {\rm Best ask})}
    \end{figure}
    
    初期デプスが確率変動すると仮定した場合の転移確率を計算する．初期デプス$R_A, R_B$が上に示すパラメータを有つ
    確率分布に従うと仮定した時，転移確率は
    \begin{align}
    	\int_{0}^{\infty} \left( \int_{1}^{\infty}\int_{1}^{\infty} f(x|r_A,r_B) d\prob{R_A-1 = r_A} d\prob{R_B-1 = r_B} \right) dx
    \end{align}
    として計算すればよい．
...
\underline{\large 転移行列}\\
    ...
    \begin{description}
        ...
        \item[$10$枚累積した時点を一回の到着として測った場合]\mbox{}\\
        	\begin{align}
    			\left(
    			\begin{array}{@{\,}cc@{\,}}
    				p_{UU} & p_{UD} \\
        ...
        \item[$30$枚累積した時点を一回の到着として測った場合]\mbox{}\\
        	\begin{align}
    			\left(
    			\begin{array}{@{\,}cc@{\,}}
    				p_{UU} & p_{UD} \\
...
\underline{\large 期待される板の変動回数}\\
	式(\refeq{eq:fluctime_TM})により変動が起こるまでの時間の平均値が計算される．変動の分布は直前の状態(上昇か下降か)に依存するから，
    考える事象は，板が上に動いた下で次に変動が起こる事象，
    板が下に動いた下で次に変動が起こる事象，の二つである．変動が起こるまでの時間の分布は，状態の分布$p_U, p_D$と式(\refeq{eq:transientprobs})の関数を用いて
    \begin{align}
    	p_U \int_{0}^{t} f_U(\tau \mid r_A^U, r_B^U) + f_D(\tau \mid r_A^U, r_B^U) d\tau
        + p_D \int_{0}^{t} f_U(\tau \mid r_A^D, r_B^D) + f_D(\tau \mid r_A^D, r_B^D) d\tau
    \end{align}
    と表現される．李\cite{li_hui_endo_kishimoto}に倣い，考えている{\rm Markov}連鎖が時間に関して一様で強定常である下では，
    $p_U, p_D$は式(\refeq{eq:stationary_dist})を用いて表現できる．変動が起こるまでの平均時間(秒)で
    セッションの長さ(秒)を割れば，そのセッションにおける期待される変動回数を計算することができる．ただし，
    モデルの上では到着率の大小関係により平均時間が有限でない場合が発生しうる．式(\refeq{eq:mean_adminishingtime})により，変動の平均時間が
    有限値を有つのは$\lambda_A < \mu_A$かつ$\lambda_B < \mu_B$の場合に限られる．\\
...
\underline{\large 転移確率から計算されるモデル上の分散と{\rm Realized volatility}との比較}\\
    ...
    簡便にするため$u$回上昇し$d$回下降したとする．$u \times d$の格子を用意して考えれば，考えるべきは左下隅のノードから右上隅のノード
...
\subsection{{\rm Bessel}関数の諸性質}
    ...
    \end{screen}
    \begin{Proof}
    	右辺の級数は$y=0$を極に有つ．積分路を原点を囲う円として$C$と表せば，留数定理により級数に於ける$1/y$の係数が
...
\label{sec:appendix_order_statistic}
    ...
    序でに密度関数も導出しておく．$F(x) \equiv \prob{X \leq x}$が密度関数$f(x)$を有つとき，
...
\subsection{負の二項分布のパラメータ推定}
    ...
    \end{align}
    と表せる．尤度関数を$p$に関して偏微分する．
    \begin{align}
    	\frac{\partial}{\partial p} L(n,p)
        &= \prod_{i=1}^{N} \binom{n+X_i-1}{X_i} \left( nNp^{nN-1} (1-p)^{X_1+X_2+\cdots+X_N} - (X_1+X_2+\cdots+X_N) p^{nN} (1-p)^{X_1+X_2+\cdots+X_N-1} \right) \\
        &= \prod_{i=1}^{N} \binom{n+X_i-1}{X_i} p^{nN-1} (1-p)^{X_1+X_2+\cdots+X_N-1} \left( nN(1-p) - (X_1+X_2+\cdots+X_N) p \right).
    \end{align}
    偏微係数を$0$にするような$\hat{p}$は
    \begin{align}
    	\hat{p} = \frac{nN}{X_1+X_2+\cdots+X_N + nN}
    \end{align}
    と計算される．$p=p(n)=nN/(X_1+X_2+\cdots+X_N + nN)$を代入して対数尤度関数を$n$に関して偏微分する．
    \begin{align}
    	\Log{L(n,p)}
        &= nN \Log{p(n)} + (X_1+X_2+\cdots+X_N)\Log{(1-p(n))} + \sum_{i=1}^{N} \left( \Log{n(n+1)(n+2)\cdots(n+X_i-1)} - \Log{X_i!} \right), \\
        \frac{\partial}{\partial n} \Log{L(n,p)}
        &= N \Log{p(n)} + nN \frac{p'(n)}{p(n)} - (X_1+X_2+\cdots+X_N) \frac{p'(n)}{1-p(n)} 
        	+ \sum_{i=1}^{N} \left( \frac{1}{n} + \frac{1}{n+1} + \cdots + \frac{1}{n+X_i-1} \right) \\
        &= N \Log{p(n)} + nN \frac{N(X_1+X_2+\cdots+X_N)}{\{(X_1+X_2+\cdots+X_N)+nN\}^2} \frac{(X_1+X_2+\cdots+X_N)+nN}{nN} \\
        	&\quad- (X_1+X_2+\cdots+X_N) \frac{N(X_1+X_2+\cdots+X_N)}{\{(X_1+X_2+\cdots+X_N)+nN\}^2} \frac{(X_1+X_2+\cdots+X_N)+nN}{(X_1+X_2+\cdots+X_N)} \\
            &\quad+ \sum_{i=1}^{N} \left( \frac{1}{n} + \frac{1}{n+1} + \cdots + \frac{1}{n+X_i-1} \right) \\
        &= N \Log{p(n)} + \sum_{i=1}^{N} \left( \frac{1}{n} + \frac{1}{n+1} + \cdots + \frac{1}{n+X_i-1} \right) \\
        &= N \left( \Log{nN} - \Log{(X_1+X_2+\cdots+X_N + nN)} \right) + \sum_{i=1}^{N} \left( \frac{1}{n} + \frac{1}{n+1} + \cdots + \frac{1}{n+X_i-1} \right).
    \end{align}
    対数尤度関数は$n$を唯一の変数とする関数で表されているから，{\rm Newton}法により対数尤度関数の導関数の零点を計算する．
    \begin{align}
    	\frac{\partial^2}{\partial n^2} \Log{L(n,p)}
        = \frac{N}{n} - \frac{N^2}{X_1+X_2+\cdots+X_N + nN} - \sum_{i=1}^{N} \left( \frac{1}{n^2} + \frac{1}{(n+1)^2} + \cdots + \frac{1}{(n+X_i-1)^2} \right).
    \end{align}
    を用いて，{\rm Newton}法の再帰計算は次のように表される．
    \begin{align}
    	n_{k+1} = n_k - \frac{\frac{\partial}{\partial n} \Log{L(n_k,p(n_k))}}{\frac{\partial^2}{\partial n^2} \Log{L(n_k,p(n_k))}}.
    \end{align}
    {\rm R}で実装したものを載せておく．
    \begin{lstlisting}[style=customR]
