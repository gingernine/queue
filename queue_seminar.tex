...
\subsection{$Random\quad Arrival$}
    \begin{description}
    	\item[ランダムな到着]\mbox{}\\
            ...
            逆に，客が$Poisson$到着する場合に各々の客がランダム到着していることを示す．$Poisson$到着の下で式(\refeq{eq:random_arrival})が成り立つことを示せばよい．\\
            ...
            \begin{align}
                ...
                &= \sum_{j=0}^{x-n} \frac{\frac{\tau^j}{j!} \frac{t^n}{n!} \frac{(T - t - \tau)^{x-n-j}}{(x-n-j)!} }{\frac{T^x}{x!}} \\
                &= \frac{t^n}{n!} \frac{x!}{T^x} \sum_{j=0}^{x-n} \frac{\tau^j}{j!} \frac{(T - t - \tau)^{x-n-j}}{(x-n-j)!} \\
                &= \frac{t^n}{n!} \frac{x!}{T^x} \frac{(T - t)^{x-n}}{(x-n)!} \\
...
\subsection{$Modified\quad Bessel\quad Function$}
    ...
    \begin{align}
        ...
        &= \exp{-(\lambda + \mu)t} \rho^{\frac{k}{2}} I_{-k}(2t\sqrt{\lambda \mu}). & (\rho \equiv \frac{\lambda}{\mu},\ I_{-k}(x) : \mbox{第一種変形$Bessel$関数}) \label{eq:prob_c}
    \end{align}
    \begin{screen}
    	\begin{description}
...
\subsection{$Finite\quad Number\quad of\quad Customers\quad in\quad the\quad System$}
    ...
    \begin{description}
    	\item[確率母関数($Probability\ generating\ function$)]\mbox{}\\
            ...
            \end{align}
        \item[確率の和]\mbox{}\\
        	$Abel$の連続性定理((\ref{sec:appendix_abel_theorem})参照)を使う．
        	\begin{align}
...
\label{sec:appendix_egorov}
	\begin{screen}
    	\begin{Lemma}
            ...
            $\rm{X}$の各点で有限な$f(x)$に収束し，また$\mu(\rm{X})<\infty$が成り立っているとき，任意の$\epsilon > 0$と$\eta > 0$
    ...
    \begin{screen}
    	\begin{Corol}
        	上の$Lemma$は，$\mathfrak{B}-\mbox{可測関数}$の列$\{f_n(x)\}_{n=1}^{\infty}$が$\rm{X}$上各点で発散するとしても成立する: \\
            任意に大きな実数$M > 0$と任意に小さな$\eta > 0$に対して或る集合$F \in \mathfrak{B}$と自然数$N$が存在して，
            \begin{align}
            	\mu(X - F) < \eta, \quad |f_n(x)| > M. \quad (\forall n \geq N,\quad \forall x \in F)
            \end{align}
        \end{Corol}
    \end{screen}
    \begin{Proof}
    	上の証明で$E_n \equiv \bigcap\limits_{k=n}^{\infty} \{x \mid |f_k(x)| > M\}$と置いて以降は全く同様である．\qed
    \end{Proof}
