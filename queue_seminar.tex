...
\underline{\large 転移確率から計算されるモデル上の分散と{\rm Realized volatility}との比較}\\
    ...
    \end{align}
    と表現することになる．上式の確率を表す記号は式(\refeq{eq:transient_probability_matrix})，(\refeq{eq:stationary_dist})のものと同じである．
    ...
    $T$ステップ後に価格が$N$ティックだけ変動する確率を計算したい．$T$は一つのセッションでの板の変動回数を表すと考えて，
    これは十分大きな自然数であるとしている．$T$ステップで動く幅は高々$T$ティックであるから$N$は$[-T, T]$の範囲に収まる．
    ...
    までの最短経路の選択方法である．格子の作り方から，格子を右方向に進行することは板が上昇する状態に対応し，格子を上方向に進行することは
    ...
    注意するべきは，格子上を動く時，初めの動きの方向が定まった下では，曲がる回数が等しい経路は通る確率が等しいということである．
    解説しておく．初めの方向選択での場合分けが必要なのは，初めの動きが{\rm Markov}連鎖の転移確率ではなく初期分布で与えられるためである．
    格子の進行の仕方を各枠の進行方向$U,D$で表現すると，$u$回上がって$d$回下がる経路は$u$個の$U$と$d$個の$D$の順列で表現できる．
    \begin{align}
    ...
    \end{align}
    と表現できる．$u-1$通りの経路ができるが，どの経路も通る確率は
    \begin{align}
    	p_U \times p_{UD}^1 p_{DU}^1 p_{UU}^{u-1-1} p_{DD}^{d-1-0}
    \end{align}
    である．$\{UUU\cdots U\}\{DDD\cdots D\}$を並べて作る列には$u+d-1$個の$U$と$D$の結合関係がある．$U$のグループも$D$のグループも分割
    しなければ，結合関係$UU$が$u-1$個，結合関係$DD$が$d-1$個，結合関係$UD$が$1$個ある．$U$のグループを二分割，$D$のグループは分割しない場合，
    $UUU\cdots U$の間に$DDD\cdots D$の塊が食い込むから，結合関係$UU$は$1$個減り$d-2$個，結合関係$DD$は$d-1$個，結合関係$UD$が$1$個，結合関係$DU$が
    $1$個となる．結合関係と転移確率は対応し，それぞれの結合関係の個数は$u-1$通りの経路で等しいから，どの経路を通る確率も等しくなるのである．
    ...
    \end{align}
    と表現できる．今度は結合関係$DD$の数が一つ減り結合関係$UD$の数が一つ増える．そして先と同じ理由でどの経路を通る確率も等しく，それは
    \begin{align}
    ...
    \end{align}
    と表現できる．今度は結合関係$UU$の数が一つ減り結合関係$DU$の数が一つ増える．そして先と同じ理由でどの経路を通る確率も等しく，それは
    \begin{align}
    ...
    \end{align}
    で与えられる．$k$をいくつまでとることができるかの問題を考えねばならない．場合分けすると，
    \begin{table}
    	\centering
        \begin{tabular}{c|c} \bhline{1.5pt}
        	場合 & 曲がることができる最大数 \\ \hline \hline
