...
\subsection{$Modified\quad Bessel\quad Function$}
    ...
    $Bessel$関数の対称性から，
    \begin{align}
    	\prob{C_{(0, t]} = k} = \rho^{k} \prob{C_{(0, t]} = -k}, \quad k=0,1,2,\cdots \label{eq:bessel_symmetry}
    \end{align}
    が成り立つ．
...
\subsection{$Transient\quad Probability$}
    ...
    \end{align}
    最終段は式(\refeq{eq:bessel_symmetry})の関係を使った．
	\begin{screen}
    	\begin{description}
...
\subsection{$Transient\quad Solution$}
    ...
    \begin{align}
        ...
        &= 1.
    \end{align}
    最後の三段の式変形は，式(\refeq{eq:bessel_symmetry})と(\refeq{eq:sum_prob_change})を使った．\\
    従って，次が成り立つ．
    \begin{align}
