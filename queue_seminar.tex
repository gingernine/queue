...
\subsection{$Random\quad Arrival$}
    \begin{description}
    	\item[ランダムな到着]\mbox{}\\
            ...
            或る時間の到着数の分布が$Poisson$分布の形で表現される．逆に，客が$Poisson$到着する場合に，一人の客が時間$(\tau, \tau + t] \subset (0, T]$に到着する確率が$\frac{t}{T}$である
            ことを示す．考えるものは，
            \begin{align}
...
\subsection{$Asymptotic\quad Distribution$}
    ...
    \begin{screen}
    	\begin{Prop}
            ...
            $\lim\limits_{t \to \infty} P_{ij}(t) = (1-\rho)\rho^j$を持ち，またこの極限分布を持つのは$\rho < 1$である場合に限られる．
        \end{Prop}
    \end{screen}
    \begin{Proof}
    	\begin{description}
        	\item[必要性]\mbox{}\\
                ...
                \begin{align}
                    ...
                    &= \frac{ \exp{-(\sqrt{\lambda}-\sqrt{\mu})^2 t} \rho^\frac{k}{2} }{ 2(\pi t)^\frac{1}{2} (\lambda \mu)^\frac{1}{4} } + o\left( \frac{1}{\sqrt{t}} \right) \label{eq:prob_c_deform}
                \end{align}
                ...
                \begin{align}
                    ...
                    &= \sum_{k=-j-i-1}^{\infty} \frac{ \exp{-(\sqrt{\lambda}-\sqrt{\mu})^2 t} \rho^\frac{k}{2} }{ 2(\pi t)^\frac{1}{2} (\lambda \mu)^\frac{1}{4} } 
                    	+ \sum_{k=-j-i-1}^{\infty} o\left( \frac{1}{\sqrt{t}} \right) \\
                    &= \frac{ \exp{-(\sqrt{\lambda}-\sqrt{\mu})^2 t} }{ 2(\pi t)^\frac{1}{2} (\lambda \mu)^\frac{1}{4} } \sum_{k=-j-i-1}^{\infty}  \rho^\frac{k}{2} 
                    	+ o\left( \frac{1}{\sqrt{t}} \right) \sum_{k=-j-i-1}^{\infty} 1 \\
                    &= \frac{ \exp{-(\sqrt{\lambda}-\sqrt{\mu})^2 t} }{ 2(\pi t)^\frac{1}{2} (\lambda \mu)^\frac{1}{4} } \frac{1}{(1-\sqrt{\rho})(\sqrt{\rho})^{j+i+1}} 
                    	+ o\left( \frac{1}{\sqrt{t}} \right) \sum_{k=-j-i-1}^{\infty} 1 & (\because |\rho| < 1)
                \end{align}
                最終段第一項は"任意の$\epsilon > 0$に対し$\forall t > T,\quad s.t.\ \mbox{(項)} < \epsilon$なる$T > 0$が存在する"ことは言える．第二項について級数部分を次のように考えればよい．\\
                \begin{description}
                	\item[]\mbox{}\\
                		いくらでも大きな自然数$M$を持ってきて，級数を$M$項まで足し合わせても，総和をはるかに凌駕する大きな$t$が存在して，第三項はいくらでも小さくすることができる．\\
                \end{description}
                \mbox{}\\
                以上の議論をまとめると次が成り立つ．
                \begin{align}
            ...
            \item[十分性]\mbox{}\\
                ...
                \begin{align}
                	(1-\rho)\rho^j K_{-j-i-2}(t) &= (1-\rho)\rho^j \sum_{k=-\infty}^{-j-i-2} \frac{ \exp{-(\sqrt{\lambda}-\sqrt{\mu})^2 t} \rho^\frac{k}{2} }{ 2(\pi t)^\frac{1}{2} (\lambda \mu)^\frac{1}{4} } 
                    	+ \sum_{k=-\infty}^{-j-i-2} o\left( \frac{1}{\sqrt{t}} \right) \\
	...
	\begin{screen}
    	\begin{description}
            ...
            \item[強定常過程$(Strictly\quad stationary\quad process)$]\mbox{}\\
            	確率過程$\{ X_t \mid t \in T \},\ (T:\mbox{時間集合})\ $が定常過程であるとする．このとき，任意の正実数$h$，任意の有限時点列
                $\{t_1, t_2, \cdots ,t_n\} \sebset T$，及び過程が取り得る任意の有限状態集合$\{s_1, s_2, \cdots ,s_n\}$に対して，
                \begin{align}
                	&\prob{X_t_1 = s_1, X_t_2 = s_2, X_t_3 = s_3, \cdots,X_t_n = s_n} \\
                    &\quad= \prob{X_{t_1+h} = s_1, X_{t_2+h} = s_2, X_{t_3+h} = s_3, \cdots,X_{t_n+h} = s_n}
                \end{align}
...
\label{sec:appendix_erlang}
	\begin{description}
		...
		\begin{align}
            ...
            &= \left| \int_{-tR}^{0} (\frac{R}{\beta}+iy)^{\alpha-1} \exp{-\frac{R}{\beta}-iy} i dy \right| \\
			&\leq \int_{-tR}^{0} (\frac{R}{\beta}+|y|)^{\alpha-1} \exp{-\frac{R}{\beta}} dy.
		\end{align}
