    ...
    \begin{Proof}
    	\begin{description}
            ...
            \item[(3)逆を示す]\mbox{}\\
                ...
                \begin{eqnarray}
                	\prob{T_n - T_{n-1} \leq t} &=& 1 - \int_{0}^{\infty} \cprob{A(\tau, \tau+t] = 0}{A_{(0, \tau]} = n-1} d\prob{A_{(0, \tau]} = n-1} \\
...
\subsection{$Blank\quad Time\quad of\quad Service$}
    ...
    \[
    	\prob{A_{(0, t]}} = \exp{-\lambda t} \frac{(\lambda t)^n}{n!}, \quad n = 0, 1, 2, \cdots
    \]
    と表される．\\
...
\subsection{$Modified\quad Bessel\quad Function$}
    ...
    \[
    	\prob{L_{(0, t]}} = \exp{-\mu t} \frac{(\mu t)^n}{n!}, \quad n = 0, 1, 2, \cdots.
    \]
    ...
    $Bessel$関数の対称性から，
    \begin{eqnarray}
    	\prob{C_{(0, t]} = k} = \rho^{k} \prob{C_{(0, t]} = -k}, \quad k=0,1,2,\cdots
    \end{eqnarray}
    が成り立つ．
...
\subsection{$Transient\quad Probability$}
    ...
    $Q(t) = \max{C(0, t] - \gamma_t}{Q(0) + C(0, t]} = \max{u - d - \gamma_t}{i + u - d}$であることから，次の関係が出る．
    \begin{eqnarray}
    	\{Q(t) \leq j \mid Q(0)=i, \alpha_t = u, \beta_t = d\} = \{u- d - j \leq \gamma_t,\ i + u - d \leq j \}. \\
    \end{eqnarray}
    $i + u - d \leq j$は$\gamma_t$に関係なく成立するから，$\sum_{<j-i>}$を$\{u,d \mid u - d \leq j - i,\ u,d \geq 0\}$を満たす$u,d$についての総和記号として，
    ...
    \begin{eqnarray}
        ...
        &\qquad-& \sum_{<j-i>} \prob{\alpha_t = u}\prob{\beta_t = d} \cprob{\gamma_t \leq u - d - j - 1}{\alpha_t = u,\ \beta_t = d} \\
        &=& \sum_{<j-i>} \exp{-\lambda t} \frac{(\lambda t)^u}{u!} \exp{-\mu t} \frac{(\mu t)^d}{d!} \\
        &\qquad-& \sum_{<j-i>} \exp{-\lambda t} \frac{(\lambda t)^u}{u!} \exp{-\mu t} \frac{(\mu t)^d}{d!} \frac{u!d!}{(d+j+1)!(u-j-1)!} \\
        &=& \sum_{<j-i>} \exp{-\lambda t} \frac{(\lambda t)^u}{u!} \exp{-\mu t} \frac{(\mu t)^d}{d!} \\
        &\qquad-& \left( \frac{\lambda}{\mu} \right)^{j+1} \sum_{(u-j-1)-(d+j+1) \leq -j-i-2} \exp{-\lambda t} \frac{(\lambda t)^{u-j-1}}{(u-j-1)!} \exp{-\mu t} \frac{(\mu t)^{d+j+1}}{(d+j+1)!}.
    \end{eqnarray}
    ここで，
    \begin{eqnarray}
    ...
    \end{eqnarray}
    最終段は$\quad \prob{C_{(0, t]} = k} = \rho^{k} \prob{C_{(0, t]} = -k}, \quad (k=0,1,2,\cdots)$の関係を使った．
	\begin{screen}
    	\begin{description}
...
\subsection{$Transient\quad Solution$}
    ...
    \begin{eqnarray}
        ...
        &=& 1.
    \end{eqnarray}
    最後の三段の式変形は，$\sum_{k=-\infty}^{\infty} \prob{C_{(0, t]} = k}$と$\prob{C_{(0, t]} = k} = \rho^{k} \prob{C_{(0, t]} = -k}$を使った．\\
    従って，次が成り立つ．
    \begin{eqnarray}
    ...
    \begin{screen}
    	\begin{description}
    		\item[有限時解($Transient\quad Solution$)]
            	有限時間内では系内客数は有限である．ここから，遷移確率は別名{\bf 有限時解}という．
    	\end{description}
    \end{screen}
