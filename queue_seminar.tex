...
\section{$Operating\quad Time\quad of\quad M/M/1$}
    ...
    \end{align}
    $T_i$の分布を求める前に，観測開始から系内客数が$0$になるまでの分布$\cprob{Q(t) < j,\ t \in (0, T_i-0]}{Q(0) = i}$を求めておく．
    \begin{align}
    ...
    \end{align}
    ところで$T_i$を意識しないで単純に計算される分布が以下の様に変形できる:
    \begin{align}
    	\cprob{Q(t) < j}{Q(0) = i} 
        &= \cprob{ C_{(0, t]} - \inf{ \tau \in (0, t] }{ C_{(0, \tau]} } < j,\ Q(0) + C_{(0, t]} < j }{Q(0) = i} \\
        &= \cprob{ C_{(0, t]} - j < \inf{ \tau \in (0, t] }{ C_{(0, \tau]} },\ i + C_{(0, t]} < j }{Q(0) = i} \\
        &= \sum_{\nu=-\infty}^{j-1} \cprob{ C_{(0, t]} - j < \inf{ \tau \in (0, t] }{ C_{(0, \tau]} },\ C_{(0, t]} = \nu-i }{Q(0) = i} \\
        &= \sum_{\nu=-\infty}^{j-1} \cprob{ \nu - i - j < \inf{ \tau \in (0, t] }{ C_{(0, \tau]} },\ C_{(0, t]} = \nu-i }{Q(0) = i} \\
        &= \sum_{\nu=-\infty}^{j-1} \cprob{ 0 < \nu - j < i + \inf{ \tau \in (0, t] }{ C_{(0, \tau]} },\ C_{(0, t]} = \nu-i }{Q(0) = i} \\
        &= \sum_{\nu=-\infty}^{j-1} {}^{o}p_{i \nu}(t).
    \end{align}
    従って，$T_i$を意識した遷移確率が式(\refeq{eq:Qtdist})を用いて表現可能となる:
    \begin{align}
    	{}^{o}p_{ij}(t) &= \sum_{\nu=-\infty}^{j} {}^{o}p_{i \nu}(t) - \sum_{\nu=-\infty}^{j-1} {}^{o}p_{i \nu}(t) \\
        &= \cprob{Q(t) < j + 1}{Q(0) = i} - \cprob{Q(t) < j}{Q(0) = i} \\
        &= \cprob{Q(t) \leq j}{Q(0) = i} - \cprob{Q(t) \leq j-1}{Q(0) = i} \\
        &= K_{j-i} - \rho^{j+1} K_{-j-i-2}(t) - (K_{j-i-1} - \rho^{j} K_{-j-i-1}(t)) \\
        &= \prob{C_{(0, t]}} - 
    \end{align}
