...
\subsection{$Asymptotic\quad Distribution\quad and\quad Stationary\quad Distribution$}
    極限分布が存在すればそれは定常分布である．$Kolmogorov$の前進方程式(式(\refeq{eq:kolmogorov_forward}))より
    \begin{align}
    	\begin{cases}
    		\frac{dP_{ij}(t)}{dt} = \mu P_{i\ j+1}(t) - (\lambda + \mu) P_{ij}(t) + \lambda P_{i\ j-1}(t) & \text{$j \geq 1$} \\
            \frac{dP_{i0}(t)}{dt} = \mu P_{i 1}(t) - \lambda P_{i0}(t) & \text{$j = 0$} \\
        \end{cases}
    \end{align}
    が成り立ち，定常分布を仮定すれば右辺の極限が存在するので左辺も極限が存在する．左辺の極限は$0$であることを示す．
    \begin{screen}
    	\begin{Prop}
    ...
    \begin{Proof}
        ...
        \qed
    \end{Proof}
    この定理から，極限分布を$p_j = \lim\limits_{t \to \infty} P_{ij}(t)\quad$(初期状態には関係ない)と表記すると，
    \begin{align}
    	\begin{cases}
    ...
    \begin{align}
        ...
        \Rightarrow G(z) &= \frac{\mu p_0}{\mu - \lambda z} = \frac{p_0}{1 - \rho z}.
    \end{align}
    最後の式変形は$z \neq 1$から成立したものである．この式を展開すると次のように表せる．
    \begin{align}
    	G(z) = \frac{p_0}{1 - \rho z} = p_0 \sum_{n=0}^{\infty} (\rho z)^n.
    \end{align}
    $\sum\limits_{j=0}^{\infty} p_j = G(1)$については
