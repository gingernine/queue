...
\section{$Density\quad Function\quad of\quad Operating\quad Time$}
...
\section{$Alignment\quad of\quad Arrivals$}
    ...
    %実データの平均到着数，平均離脱数を示す．
    %実データの時間間隔のヒストグラムを示す．
    実際にシステム，つまり最良気配にかかる注文数はまとまって来る場合が多い．
    指数分布の無記憶性から到着は
...
\section{$Distribution\quad of\quad Sum\quad of\quad Poisson\quad Random\quad Variables$}
	$X(\omega),\ Y(\omega)$をパラメータ$\lambda > 0$の$Poisson$分布に独立に従う確率変数であるとする．
    ...
    \end{align}
    従って，和の分布はパラメータ$2\lambda$の$Poisson$分布に従うとわかる．機能的に考えて，任意の$s$個の独立で同一な$Poisson$分布$P_O(\lambda)$に従う確率変数
